...
def do_test(cfg, model):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        train_data = load_json(cfg.DATASETS.TRAINPATH)
        names = [i['name'] for i in train_data['categories']]
        del train_data
        DatasetCatalog.register(cfg.DATASETS.TEST[0], lambda: preprocess_coco(cfg.DATASETS.TESTPATH))  # for test
        MetadataCatalog.get(cfg.DATASETS.TEST[0]).set(thing_classes=names, evaluator_type='coco')
...
def do_train(cfg, model, resume=False, train_only=False):
    model.train()
    ...
    names = [i['name'] for i in train_data['categories']]
    del train_data
    DatasetCatalog.register(cfg.DATASETS.TRAIN[0], lambda: preprocess_coco(cfg.DATASETS.TRAINPATH))
    DatasetCatalog.register(cfg.DATASETS.TEST[0], lambda: preprocess_coco(cfg.DATASETS.TESTPATH))  # for test
...
def main(args):
    ...
    do_train(cfg, model, resume=args.resume, train_only=args.train_only)
    return do_test(cfg, model) if not args.train_only else None
...
""",
    ...
    parser.add_argument("--config-file",
                        default="/home/RecycleDetection/models/CenterNet2/configs/CenterNet2_R2-101-DCN-BiFPN_4x+4x_1560_ST.yaml",
